...
- Fish is a `CarryableInteractable` with `carryId = fish`. The fishsign `CarryDropZone` requires `picture_cookie_delivered_to_rcat`; pressing `E` there while carrying fish writes `picture_fish_delivered_to_fishsign`, destroys the dropped fish, hides the fishsign placeholder and Lcat, reveals gift, and assigns `mission_picture_take_birthday_gift`.
...
`BedEndingBookSequence` is specific to `Assets/_Project/Scenes/BedRoomEnding.unity`. It is attached to the `Book` object and runs an `E`-stepped ending beat: hold the book on the first open-animation frame, press `E` to play `AnimationBookOpen`, press `E` again to reveal `Ảnh 1_0` with a memory-style fade/scale, press `E` again to reveal `Ảnh 2_0`, then press `E` again to fade both memories out before setting Animator bool `isFlip` and playing `BookFlip`. After the page flip, the two TMP ending texts reveal from left to right with `maxVisibleCharacters`, in serialized order, and `E` returns to the main menu once the return prompt is visible.
